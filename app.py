...
#Flask setup and Flask routes with all available routes shown
app = Flask()
...
def precipitation():
    # Create session from Python to databse
...
#Query station names and their observation counts sorted descending and select most active station
    ...
    station_mo = q_station_list[0][0]
    print(station_mo)

#Return a list of tobs for the year before the final date
    results_ybf = (session.query(Measurement.station, Measurement.date, Measurement.tobs)
                      .filter(Measurement.date >= query_start_date)
                      .filter(Measurement.station == station_mo)
                      .all())

#Create JSON results
    tobs_list = []
    for result in tobs_list:
        line = {}
        line["Date"] = result[1]
...
#Find the min tempurature, average tempurature, and max tempurature for given start or start-end range
